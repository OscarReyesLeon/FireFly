...
def process_sensor(request):
    ...
    lectura = LecturaModel.objects.filter(
        **dict_filter).using('sensor').values("fecha", "maquina", "maquina__nombre", "valor")
    ...
    lectura2 = (lectura_values.groupby(['maquina',] + extra_group)['valor']
            .mean()
            .unstack('maquina')
            .fillna(0)
            .round(2)
    ...
    if export_report in ['1','true']:
            ...
            lectura2.to_csv(path_or_buf=response)
            return response
